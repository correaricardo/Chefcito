...
use Chefcito::Chef::Recipe;
use Chefcito::Chef::Container;
use Chefcito::Chef::Ingredient;

=head1 NAME

Chefcito::Chef - An interpreter for the Chef programming language
...
=head1 SYNOPSIS
  ...
  my $compiled = Chefcito::Chef->compile($code_string);
...
All methods provided by Chefcito::Chef are adequately described in the
...
string is returned that reconstructs the Chefcito::Chef
object.
...
sub _paragraphsToRecipes {
   ...
   while (@$paragraphs) {
      ...
      push @recipes, Chefcito::Chef::Recipe->new(
        name         => $recipe_name,
        comments     => $comments,
